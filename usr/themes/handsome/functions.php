...
function themeConfig($form) {
    ...
    $indexsetup = new Checkbox('indexsetup',
        array(
            ...
            'smallStyleThumb' => _t('首页头图为缩小版头图样式'),
...
是一种新的图片格式，最多可以减少70%的图片大小，目前只有chrome内核比较支持，safari和firefox 是无法显示图片的。</br>有些云服务商 <code>webp自适应开关</code>，开启后无需启用主题的 <code>启用 WebP 图像格式</code>选项，在支持的浏览器自动转换webp格式。 </br>具体细节请看 <a href=\"https://handsome.ihewro.com/#/functions\">详细使用文档</a>");
    ...
    $publicCDNSelcet = new Select('publicCDNSelcet',array(
        ...
        '4' => 'jsDelivr',
        '5' => 'catCDN'
    ),'1',_t('选择公共CDN库'),_t('主题除了使用本地一些静态资源，还引用了bootstrap/jquery 库的公共cdn地址（体积较大>60kb），通常使用托管在公共cdn的资源会获得更快的访问速度</br>默认选择BootCDN，因为各大公共CDN库的目录结构有所差异，暂不支持自定义CDN库。
<br/><span style="color: red">注意：bootcdn在有些地区可能无法正常访问，此时你可以更换其他的公共CDN，比如百度或七牛，稳定性比较好。</span>'));
   ...
   // $form->addItem(new Typecho_Widget_Helper_Layout("div",array("class"=>"titleStart")));
    ...
    $adContentSidebar = new Textarea('adContentSidebar', NULL,NULL,_t('全局右侧边栏广告位'),_t('此处可以填写HTML代码，广告位的位置在右侧的边栏中'));
...
{   $options = Typecho_Widget::widget('Widget_Options');
    ...
    $html_7days = "<h3>本周评论排行榜</h3>";
    if (count($result_7days) > 0) {
        $html_7days .= '<ul class=\'readers-list\'>';
        foreach ($result_7days as $value) {
            $html_7days .= '<li><a target="_blank" rel="nofollow" href="' . $value['url'] . '"><span class="pic" style="background: url(https://secure.gravatar.com/avatar/'.md5(strtolower($value['mail'])).'?s=36&d=&r=G) no-repeat;background-size: 36px;"></span><em>' . $value['author'] . '</em><strong>+' . $value['cnt'] . '</strong><br />' . $value['url'] . '</a></li>';
        }
        $html_7days .= '</ul>';
        echo $html_7days;
    }else{
    ...
    $mostactive = "<h3>总评论排行榜</h3>";
    if (count($result) > 0) {
        $mostactive .= '<ul class=\'readers-list\'>';
